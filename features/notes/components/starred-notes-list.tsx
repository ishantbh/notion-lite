...
export async function StarredNotesList({ userId, currentPage, query }: Props) {
      ...
      {userStarredNotes.length ? (
        ...
        <div className='flex flex-col items-center justify-center text-center gap-4 grow opacity-80'>
          ...
          <h2 className='text-xl sm:text-2xl mb-2 font-medium text-muted-foreground'>
            {query ? (
              <span>No starred notes found matching "{query}"</span>
            ) : (
              <span>You don't have any starred notes yet</span>
            )}
          </h2>
        </div>
      )}
